...
local try_apply_outline_to_players = function ()
    repeat
        if not kUtil.entity_manager then
            k_log("[TEST] missing entity_manager !!!!")
            break
        end
        ...
        if my_player then
            ...
            my_team = team_ext.internal and team_ext.internal.team or 1
        end
        ...
        for _, player in pairs(players) do
            ...
            apply_game_outline(player.unit, true, is_friend and {0, 1, 0.25} or {1, 0, 0.25})
        end
    until true
end
...
outline_async_loop = function ()
    ...
    kUtil.task_scheduler.add(outline_async_loop, 5000)
end
